'use strict';

describe('Controller Tests', function() {

    describe('Terreno Management Detail Controller', function() {
        var $scope, $rootScope;
        var MockEntity, MockPreviousState, MockTerreno, MockCaracteristicasTerreno, MockServiciosTerreno, MockLocalizacionesTerreno;
        var createController;

        beforeEach(inject(function($injector) {
            $rootScope = $injector.get('$rootScope');
            $scope = $rootScope.$new();
            MockEntity = jasmine.createSpy('MockEntity');
            MockPreviousState = jasmine.createSpy('MockPreviousState');
            MockTerreno = jasmine.createSpy('MockTerreno');
            MockCaracteristicasTerreno = jasmine.createSpy('MockCaracteristicasTerreno');
            MockServiciosTerreno = jasmine.createSpy('MockServiciosTerreno');
            MockLocalizacionesTerreno = jasmine.createSpy('MockLocalizacionesTerreno');
            

            var locals = {
                '$scope': $scope,
                '$rootScope': $rootScope,
                'entity': MockEntity,
                'previousState': MockPreviousState,
                'Terreno': MockTerreno,
                'CaracteristicasTerreno': MockCaracteristicasTerreno,
                'ServiciosTerreno': MockServiciosTerreno,
                'LocalizacionesTerreno': MockLocalizacionesTerreno
            };
            createController = function() {
                $injector.get('$controller')("TerrenoDetailController", locals);
            };
        }));


        describe('Root Scope Listening', function() {
            it('Unregisters root scope listener upon scope destruction', function() {
                var eventType = 'hipotecasApp:terrenoUpdate';

                createController();
                expect($rootScope.$$listenerCount[eventType]).toEqual(1);

                $scope.$destroy();
                expect($rootScope.$$listenerCount[eventType]).toBeUndefined();
            });
        });
    });

});
